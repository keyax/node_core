...
RUN ["/bin/bash", "-c", "curl -SLO https://nodejs.org/dist/v$NODE_VERSION/SHASUMS256.txt.asc"]
RUN ["/bin/bash", "-c", "gpg --batch --decrypt --output SHASUMS256.txt SHASUMS256.txt.asc"]
